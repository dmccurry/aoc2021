...
fs.readFile('./input', 'utf-8', (err, data) => {
  ...
  for (let i=0; i<found.length; i++) {
    for (let j=0; j<found.length; j++) {
      if (j != i) {
        let md = Math.abs(found[i].position.x - found[j].position.x) + Math.abs(found[i].position.y - found[j].position.y) + Math.abs(found[i].position.z - found[j].position.z);
        if (md > l) {
          l = md;
        } 
      }
